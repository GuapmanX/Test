...
class User:
    def __init__(self):
        ...
        #bandit data
        self.newsl_type_data = np.ones(2, dtype=np.float64)
        self.newsl_type_pulls = np.ones(2, dtype=np.int32)
...
nl_picker = MultiArmedBandit(2,0.1)
email_picker = MultiArmedBandit(3,0.1)
...
for i in range(training_iterations):
    guess = nl_picker.selectArm(single_user.newsl_type_data)
    newsl_reward = single_user.calculate_newsletter(guess)
    single_user.newsl_type_data, single_user.newsl_type_pulls = nl_picker.update(guess, newsl_reward, single_user.newsl_type_data, single_user.newsl_type_pulls)
    
    #print(single_user.pNv_successes[guess],single_user.pNv_failiures[guess])
    email_guess = email_picker.selectArm(single_user.pNv_data[guess])
    content_reward = single_user.calculate_reward(newsl_reward, guess,email_guess)
    single_user.pNv_data[guess], single_user.pNv_pulledcount[guess] = email_picker.update(email_guess, content_reward, single_user.pNv_data[guess], single_user.pNv_pulledcount[guess])
